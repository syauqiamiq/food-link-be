...
describe("[GET] /stand-admin/promo", () => {
	it("should 200 if no problem", async () => {
		// Mock the getSessionAccessData function if needed
		const response = await request(app)
			.get("/stand-admin/promo") // replace with your actual API endpoint
	...
	it("should return 401 if unauthenticated", async () => {
		...
		const response = await request(app).get("/stand-admin/promo").expect(401);
	});
});
...
describe("[GET] /stand-admin/product/:id", () => {
	it("should 200 if no problem", async () => {
		// Mock the getSessionAccessData function if needed
		const response = await request(app)
			.get("/stand-admin/promo/1") // replace with your actual API endpoint
			.set("Authorization", `Bearer ${process.env.TEST_BEARER_TOKEN}`)
			.expect(200);
	...
	it("should return 200 and empty array data if not found", async () => {
		...
		const response = await request(app)
			.get("/stand-admin/promo/99999999999999999")
			.set("Authorization", `Bearer ${process.env.TEST_BEARER_TOKEN}`)
			.expect(200);
	...
	it("should return 401 if unauthenticated", async () => {
		...
		const response = await request(app).get("/stand-admin/promo/1").expect(401);
	});
});

// STORE PRODUCT
describe("[POST] /stand-admin/promo", () => {
	it("should 200 if no problem", async () => {
		exampleData = {
			name: "Promo Menari",
			discount_price: 5000,
		...
		// Mock the getSessionAccessData function if needed
		const response = await request(app)
			.post("/stand-admin/promo") // replace with your actual API endpoint
			.set("Authorization", `Bearer ${process.env.TEST_BEARER_TOKEN}`)
			.send(exampleData)
			.expect(200);
